...
calculate_mass_balance	<- function(year, well, sample, sample_id, pH, conc, conc_err, d15N, d15N_err, d18O, d18O_err, compound)
{	group_by(year, well, sample, pH, sample_id) %>%
		...
		do({
			...
			if (nrow(NO3) > 1) {
				...
				NO3 <- NO3 %>% group_by(sample_id, year, well, pH, sample, compound) %>% summarize_all(funs(mean(.))) %>% ungroup()
			}
			if (nrow(NOx) > 1) {
				# average multiple NO3+NO2 analyses
				NOx <- NOx %>% group_by(sample_id, year, well, pH, sample, compound) %>% summarize_all(funs(mean(.))) %>% ungroup()
			}
			if (nrow(Ntot) > 1) {
				# average multiple Ntot analyses
				Ntot <- Ntot %>% group_by(sample_id, year, well, pH, sample, compound) %>% summarize_all(funs(mean(.))) %>% ungroup()
			}
			
			# Norg entry
			if (nrow(Ntot) == 1 && nrow(NOx) == 1) {
				Norg <- 
					data_frame(
						# metatdata
						compound = "Norg",
						well = Ntot$well[1],
						sample = Ntot$sample[1],
						year = Ntot$year[1],
						pH = Ntot$pH[1],
						sample_id = Ntot$sample_id[1],
						# calculate Norg by mass balance
						conc = Ntot$conc - NOx$conc,
						conc_err = sqrt(Ntot$conc_err^2 + NOx$conc_err^2)
					)
				...
				if (Norg$conc - Norg$conc_err > 0) {
					Norg <- Norg %>%
						...
						mutate(
							d15N = iso_mass_balance(Ntot$conc, Ntot$d15N, -1 * NOx$conc, NOx$d15N),
							d15N_err = iso_mass_balance_error(
								Ntot$conc, Ntot$d15N, -1 * NOx$conc, NOx$d15N, 
								Ntot$conc_err, Ntot$d15N_err, NOx$conc_err, NOx$d15N_err)
						)
				}
