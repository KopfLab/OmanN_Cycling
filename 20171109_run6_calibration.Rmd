---
title: 'Run 6: Oman 2017 NO$_x$ $\delta^{15}N$ and $\delta^{18}O$'
subtitle: "Source file: 20171109_run6_calibration.Rmd"
date: "`r Sys.Date()`"
output:
  html_document: 
    css: stylesheet.css
...
#omit any files with problems and export to an .rds file
  raw_data %>%
    ...
    iso_save("data/raw/run6_raw") 
```

### Load raw data from .rds file
```{r "load run"}
#load entire run
raw_data <- iso_read_continuous_flow(file.path("data","raw","run6_raw.cf.rds"))
```
...
df.raw <- raw_data %>% 
  ...
  filter(category %in% c("Background", "Degassed", "N2O", "Conditioner", "OM17", "USGS34", "IAEA-NO3")) 
...
Note: Run 6 is split into subruns 3 and 4. Subrun 4 is made up of nitrite removed samples and has "NR" labeled in the sample names. The entire run was for a target N concentration of 2 nmoles.
...
plot_raw_standards <- df.raw %>%
  ...
  shape = 21, fill = subrun, size = area,
  panel = category ~ .
)
...
```{r "remove problematic data points"}
 df.raw <- df.raw %>% 
 change_category(run_number %in% c(35), "excluded") #  35 looks off in d45 compared to other standards
```
...
```{r}
df.drift <- df.raw %>% evaluate_drift(d45, d46, correct = TRUE, method = "lm", correct_with = category %in% c("USGS34", "IAEA-NO3", "N2O"), plot = TRUE)
...
stds_isotopes <- read_excel(file.path("data", "standards", "standards_run6.xlsx"), sheet = "isotopes")
stds_concs <- read_excel(file.path("data", "standards", "standards_run6.xlsx"), sheet = "concentrations")
kable(stds_isotopes)
kable(stds_concs)
```
...
```{r "exclude samples with > 10 nmoles N"}
#exclude samples that have >10 nmoles predicted N
df_w_conc <- df_w_conc %>% 
 change_category(run_number == 103, "excluded") #area too large
```
...
```{r warning = FALSE}
# apply multiple regression calibration with volume for N for subrun 3
df_finalN1 <- df_calibN %>%
filter(d15N_calib == "Nwith_volume")%>% 
	filter (subrun == "subrun 3") %>%
  iso_apply_calibration(
    predict = true_d15N,
    calibration = "d15N",
...
# apply multiple regression calibration with volume for N for subrun 4
df_finalN2 <- df_calibN %>%
filter(d15N_calib == "Nwith_volume")%>% 
	filter (subrun == "subrun 4") %>%
  iso_apply_calibration(
    predict = true_d15N,
    calibration = "d15N",
    calculate_error = TRUE
...
df_finalN <- bind_rows(df_finalN1, df_finalN2)
  
# apply multiple regression calibration with volume for O for subrun 3
...
# apply multiple regression calibration with volume for O for subrun 4
...
df_finalO <- bind_rows(df_finalO1, df_finalO2)
...
#ggplot version with check if calibration us ok
...
```{r, include=FALSE}
df_final %>%
  filter(category == "OM17" | category == "soil") %>%
...
write.csv(file = file.path("data", "calibrated", "run6_calib_data_export.csv"))

df_final %>% 
 filter(category == "OM17") %>% 
  saveRDS(file = file.path("data", "calibrated", "df_final_run6"))
# saveRDS
``` 
...
write.csv(file = file.path("data", "calibrated", "run6_stds_calib_data_export.csv"))

df_final %>% 
 filter(category %in% c("USGS34", "IAEA-NO3")) %>%
  saveRDS(file = file.path("data", "calibrated", "df_stds_run6"))
# saveRDS
```
