---
title: 'Run 7: Oman 2017 NO$_x$ $\delta^{15}N$ and $\delta^{18}O$'
subtitle: "Source file: 20171110_run7_calibration.Rmd"
date: "`r Sys.Date()`"
output:
  html_document: 
    css: stylesheet.css
...
The combined dissolved NO$_3$ and NO$_2$ isotopic composition was measured using the denitrifier method coupled to GC-IRMS as described in the methods section of the manuscript. The following code steps through the calibration of measured isotopic values. The fluid samples processed here in this document were collected in Oman in the January 2017 field season. 
...
#omit any files with problems and export to an .rds file
  raw_data %>%
    ...
    iso_save("data/raw/run7_raw") 
```

### Load raw data from .rds file
```{r "load run"}
#load entire run
raw_data <- iso_read_continuous_flow(file.path("data", "raw", "run7_raw.cf.rds"))
```
...
df.raw <- raw_data %>% 
  ...
  left_join(read_excel(file.path("data", "metadata", "201711_category_metadata.xlsx")) %>% select(-col_total), by = c("analysis")) %>% 
  ...
  filter(category %in% c("Background", "Degassed", "N2O", "Conditioner", "OM17", "USGS34", "IAEA-NO3", "USGS40", "USGS65")) 
...
Note: Run 7 is split into subruns: 5, 6, and 7. Subrun 5 had a target nmole concentration of 2. Subruns 6 and 7 had target nmole concentrations of 20. Subrun 7 is made up of nitrite removed samples and has "NR" written in the sample names. 
```{r "raw data summary"}
...
```{r "remove problematic data points"}
 df.raw <- df.raw %>% 
 change_category(run_number %in% c(43, 53), "excluded") #d45 outliers
```
...
### Drift correction
Evaluate drift throughout run duration. No visible drift is observed; linear correction NOT applied.
```{r}
df.drift <- df.raw %>% evaluate_drift(d45, d46, correct = FALSE, method = "lm", correct_with = category %in% c("USGS34", "IAEA-NO3", "N2O"), plot = TRUE)
...
stds_isotopes <- read_excel(file.path("data", "standards", "standards_run7.xlsx"), sheet = "isotopes")
stds_concs <- read_excel(file.path("data", "standards", "standards_run7.xlsx"), sheet = "concentrations")
stds_isotopes
stds_concs
```
...
df_calib <- df_calibp %>% 
  ...
  iso_prepare_for_calibration(group_by = subrun) %>% #generate calibrations for each subrun
...
### Inspect calibration
Residuals ~2.5% for 20 nmole stds; ~5% for 2 nmole stds
```{r warning = FALSE}
...
conc_resid <- df_calib %>% 
  ...
  mutate(
	...
	ggplot(
		aes(x = run_number, y =`Var: rel. residual area [%]`, color = subrun)) +
			geom_point(size = 4) +
			theme_figure() +
	scale_color_manual(values = cbPalette) +
			xlab("Run Number") +
...
### Apply calibration
```{r warning = FALSE}
# apply concentration calibration for subrun 5
df_w_conc1 <- df_calib %>% 
	filter (subrun == "subrun 5") %>%
...
# apply concentration calibration for subrun 6
df_w_conc2 <- df_calib %>% 
	filter (subrun == "subrun 6") %>%
  iso_apply_calibration(
    predict = amount.nmol,
    calibration = "area",
    calculate_error = TRUE, #standard error using Wald method
...
# apply concentration calibration for subrun 7
df_w_conc3<- df_calib %>% 
	filter (subrun == "subrun 7") %>%
  iso_apply_calibration(
    predict = amount.nmol,
    calibration = "area",
    calculate_error = TRUE, #standard error using Wald method
...
#combine dataframes post separate calibration
df_w_conc <- bind_rows(df_w_conc1, df_w_conc2, df_w_conc3)
...
### Check data
A concentration control was not used in calibration, but is calculated to be 167 uM +/- 1.04 uM. The known value is 161 uM
```{r}
...
```{r "exclude samples with >25 nmoles N"}
#exclude samples that have <5 nmoles predicted N
df_w_conc <- df_w_conc %>%
	filter(
		if_else(is_sample == TRUE, amount.nmol_pred < 25, amount.nmol_pred > 0)
	)
df_w_conc
```
...
df_w_conc %>% filter(category != "N2O") %>%
  group_by(subrun, category, true_conc.uM) %>% 
...
```{r warning = FALSE}
df_calibN <- df_w_conc %>% 
  iso_prepare_for_calibration(group_by = subrun) %>% 
  iso_generate_calibration(
    model = c(
...
N_residuals <- df_calibN %>%
	...
	filter(category != "excluded") %>% #filter for only standards used in calibration
	ggplot(
		aes(x = run_number, y = d15N_resid, color = d15N_calib, shape = subrun)) +
			geom_point(size = 4) +
			theme_figure() +
	scale_color_manual(values = cbPalette) +
			xlab("Run Number") +
...
#multiple regressions using concentration or volume are better than the simple linear calibration model
```
...
O_residuals <- df_calibO %>%
	...
	filter(category != "excluded") %>% #filter for only standards used in calibration
	ggplot(
		aes(x = run_number, y = d18O_resid, color = d18O_calib, shape = subrun)) +
			geom_point(size = 4) +
			theme_figure() +
	scale_color_manual(values = cbPalette) +
			xlab("Run Number") +
...
```{r warning = FALSE}
# apply multiple regression calibration with volume for N for subrun 5
...
# apply multiple regression calibration with volume for N for subrun 6
...
# apply multiple regression calibration with volume for N for subrun 7
df_finalN3 <- df_calibN %>%
filter(d15N_calib == "Nwith_volume")%>% 
	filter (subrun == "subrun 7") %>%
  iso_apply_calibration(
    predict = true_d15N,
...
df_finalN <- bind_rows(df_finalN1, df_finalN2, df_finalN3)
  
# apply multiple regression calibration with volume for O for subrun 5
df_finalO1 <- df_calibO %>% 
...
# apply multiple regression calibration with volume for O for subrun 6
df_finalO2 <- df_calibO %>% 
  filter(d18O_calib == "Owith_volume") %>% 
	filter(subrun == "subrun 6") %>%
  iso_apply_calibration(
    predict = true_d18O,
...
# apply multiple regression calibration with volume for O for subrun 7
df_finalO3 <- df_calibO %>% 
  filter(d18O_calib == "Owith_volume") %>% 
	filter(subrun == "subrun 7") %>%
  iso_apply_calibration(
    predict = true_d18O,
...
df_finalO <- bind_rows(df_finalO1, df_finalO2, df_finalO3)
...
df_final %>% 
	filter(category != "N2O") %>%
  group_by(subrun, category, true_d15N, true_d18O) %>% 
...
df_final <- df_final %>%
  mutate(
    ...
    compound = ifelse((grepl("NR", name)), "NO3", "NO3+NO2")
  )  
...
df_final %>%
  ...
  select(analysis, run_number, volume, area, name, pH, category, dilution, col_NOx, col_NO2, noxbox_Ntotal, conc, conc_err, d15N, d15N_err, d18O, d18O_err, compound) %>%
write.csv(file = file.path("data", "calibrated", "run7_calib_data_export.csv"))

df_final %>% 
 filter(category == "OM17") %>% 
  saveRDS(file = file.path("data", "calibrated", "df_final_run7"))
# saveRDS
``` 
...
```{r, include=FALSE}
df_final %>%
  filter(category %in% c("USGS34", "IAEA-NO3")) %>%
  select(analysis, run_number, volume, area, name, category, conc, conc_err, d15N, d15N_err, d18O, d18O_err, compound) %>%
write.csv(file = file.path("data", "calibrated", "run7_stds_calib_data_export.csv"))

df_final %>% 
 filter(category %in% c("USGS34", "IAEA-NO3")) %>%
  saveRDS(file = file.path("data", "calibrated", "df_stds_run7"))
# saveRDS
```
