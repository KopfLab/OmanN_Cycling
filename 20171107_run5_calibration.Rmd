---
title: 'Run 5: Oman 2017 Organic N $\delta^{15}N$'
subtitle: "Source file: 20171107_run5_calibration.Rmd"
date: "`r Sys.Date()`"
output:
  html_document: 
    css: stylesheet.css
...
#omit any files with problems and export to an .rds file
  raw_data %>%
    ...
    iso_save("data/raw/run5_raw") 
```

### Load raw data from .rds file
```{r "load run"}
#load entire run
raw_data <- iso_read_continuous_flow(file.path("data","raw", "run5_raw.cf.rds"))
```
...
df.raw <- raw_data %>% 
  ...
  mutate(
  	# add batch columns
    ox_batch = ifelse(run_number > 133, "batch 1", "batch 2"),
    volume = ifelse(category == "N2O"| category == "Conditioner", NA, volume),
  ...
  filter(category %in% c("Background", "Degassed", "N2O", "Conditioner", "OM17", "USGS34", "IAEA-NO3", "USGS40", "USGS65", "oxidationblank")) 
...
df.raw <- df.raw %>%
 # backgrounds (these are NOT used in the calibration calculations but are good for sanity checks)
	calculate_background(area, crit = grepl("Background", name)) %>% 
  calculate_oxidation_blank(area, volume, crit = grepl("ox blank", name))  
...
First set of USGS40 samples from oxidation batch 2 has issues
```{r "plot standards", warning= FALSE, plotly=TRUE}
plot_raw_standards <- df.raw %>%
	#select just isotopic and N2O standards
	filter(category %in% c("IAEA-NO3", "USGS34", "USGS40", "USGS65", "N2O")) %>%
...
```{r "remove problematic data points"}
 df.raw <- df.raw %>% 
 change_category(run_number %in% c(25, 26, 27, 140), "excluded") #  27 is too large; 25, 26, 140 too small 
#issues with sampling on mass spec?
```
...
stds_concs <- read_excel(file.path("data", "standards", "standards_run5.xlsx"), sheet = "concentrations")
kable(stds_concs)
```
...
df_calib <- df_calibp %>% 
  ...
  iso_generate_calibration(
    model = lm(area ~ amount.nmol - 1, na.action=na.omit), #linear model where area is the response variable; the - 1 removes the implied intercept term
    calibration = "area",
...
conc_resid <- df_calib %>% 
  ...
  mutate(
	...
	ggplot(
		aes(x = run_number, y =`Var: rel. residual area [%]`, color = ox_batch, shape = subrun)) +
			geom_point(size = 4) +
			theme_figure() +
	scale_color_manual(values = cbPalette) +
			xlab("Run Number") +
...
## Yields
Samples were all less than ~50 uM effective concentration after dilution with persulfate solution, so oxidation should have been efficient.
```{r}
#calculate yields
...
p <- df_w_conc %>% 
  filter(category %in% c("IAEA-NO3", "USGS34", "USGS40", "USGS65")) %>% 
  ggplot() +
...
## Isotopic Calibration with isorunN2O
...
one_step <- df_w_conc %>% 
  ...
  calibrate_d15_org(d15.raw, area, volume, standards = c("USGS40" = -4.50, "USGS65" = 20.68)) %>% 
...
two_step <- df_w_conc %>% 
  ...
  calibrate_d15(
    ...
    d15.raw, area, standards = c("USGS34" = -1.8, "IAEA-NO3" = 4.7)) %>%
  # followed by organic d15 calibration
  calibrate_d15_org(
    # now use the already nitrate calibrated d15 as starting point
    d15.cal, area, volume, standards = c("USGS40" = -4.50, "USGS65" = 20.68),
...
write.csv(file = file.path("data", "calibrated", "run5_calib_data_export.csv"))

df_final %>% 
 filter(category == "OM17") %>% 
  saveRDS(file = file.path("data", "calibrated", "df_final_run5"))
# saveRDS
``` 
...
```{r, include=FALSE}
df_final %>%
  filter(category %in% c("USGS34", "IAEA-NO3", "USGS40", "USGS65")) %>%
  mutate(
    conc = conc_uM,
    conc_err = conc_uM_err,
    compound = "NO3+Norg"
  ...
  select(analysis, run_number, volume, area, name, category, conc, conc_err, d15N, d15N_err, compound) %>%
write.csv(file = file.path("data", "calibrated", "run5_stds_calib_data_export.csv"))

df_final %>% 
 filter(category %in% c("USGS34", "IAEA-NO3", "USGS40", "USGS65")) %>%
  saveRDS(file = file.path("data", "calibrated", "df_stds_run5"))
# saveRDS
```
