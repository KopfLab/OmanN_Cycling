---
title: 'Run 4: Oman 2017 NO$_x$ $\delta^{15}N$ and $\delta^{18}O$'
subtitle: "Source file: 20170321_run4_calibration.Rmd"
date: "`r Sys.Date()`"
output:
  html_document: 
    css: stylesheet.css
...
#omit any files with problems and export to an .rds file
  raw_data %>%
    ...
    iso_save("data/raw/run4_raw") 
```

### Load raw data from .rds file
```{r "load run"}
#load entire run
raw_data <- iso_read_continuous_flow(file.path("data","raw", "run4_raw.cf.rds"))
```
...
df.raw <- raw_data %>% 
  ...
  left_join(
    read_excel(file.path("data", "metadata", "20170321_category_metadata.xlsx")) %>% select(-name), by = c("analysis"))%>% 
  # convert volumes from uL to mL 
  mutate(
    #add big picture categories
    is_standard = str_detect(name, "(USGS|IAEA)"),
    is_sample = category %in% c("OM17"),
    #samples with NR are nitrate only
    compound = case_when(grepl("NR", name) ~ "NO3", TRUE ~ "NO3+NO2"), 
    id = sub("NR", "", name),
...
Note: Run 4 is split into subruns 1 and 2. Subrun 2 is made up of nitrite removed samples and has "NR" labeled in the sample names. The entire run was for a target N concentration of 20 nmoles.
...
ggplotly(plot_raw_standards)
```

### Identify problematic data points
```{r "remove problematic data points"}
 df.raw <- df.raw %>% 
 change_category(run_number %in% c(91, 102), "excluded") #area out of range
```
...
### Drift correction
Evaluate drift throughout run duration. No drift is observed; linear correction NOT applied.
```{r}
df.drift <- df.raw %>% evaluate_drift(d45, d46, correct = FALSE, method = "lm", correct_with = category %in% c("USGS-34", "IAEA-NO3", "N2O"), plot = TRUE)
...
stds_isotopes <- read_excel(file.path("data", "standards", "standards_run4.xlsx"), sheet = "isotopes")
stds_concs <- read_excel(file.path("data", "standards", "standards_run4.xlsx"), sheet = "concentrations")
kable(stds_isotopes)
kable(stds_concs)
```
...
df_calib <- df_calibp %>% 
  ...
  iso_prepare_for_calibration(subrun) %>% 
...
### Apply calibration
```{r warning = FALSE}
# apply concentration calibration for subrun 1
df_w_conc1 <- df_calib %>% 
...
# apply concentration calibration for subrun 2
df_w_conc2 <- df_calib %>% 
...
df_w_conc <- bind_rows(df_w_conc1, df_w_conc2)

df_w_conc
```

## Check data
A concentration control was not used in calibration, but is calculated to be 164 uM +/- 7 uM. The known value is 161 uM
```{r}
...
# apply multiple regression calibration with concentration for O for subrun 1
df_finalO1 <- df_calibO %>% 
  filter(d18O_calib == "Owith_conc") %>% 
	filter(subrun == "subrun 1") %>%
  iso_apply_calibration(
    predict = true_d18O,
    calibration = "d18O",
...
# apply multiple regression calibration with conc for O for subrun 2
...
df_final %>%
  ...
  select(analysis, run_number, volume, area, name, pH, category, dilution, noxbox_conc, conc, conc_err, d15N, d15N_err, d18O, d18O_err, compound) %>%
write.csv(file = file.path("data","calibrated", "run4_calib_data_export.csv"))
