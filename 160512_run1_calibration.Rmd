---
title: 'Run 1: Oman 2015 + 2016 NO$_x$ $\delta^{15}N$ and $\delta^{18}O$'
subtitle: "Source file: 160512_run1_calibration.Rmd"
date: "`r Sys.Date()`"
output:
  html_document: 
    css: stylesheet.css
...
#omit any files with problems and export to an .rds file
  raw_data %>%
    ...
    iso_save("data/raw/run1_raw") 
```

### Load raw data from .rds file
```{r "load run"}
#load entire run
raw_data <- iso_read_continuous_flow(file.path("data","raw", "run1_raw.cf.rds"))
```
...
df.raw <- raw_data %>% 
  ...
  rename(d45 = `d 45N2O/44N2O`, d46 = `d 46N2O/44N2O`, area = `Intensity All`, dilution = `Sample Dilution`) %>%
  ...
  mutate(category = extract_word(name, capture_n = 1, include_dash = TRUE),
    ...
    is_sample = category %in% c("OM15", "OM16"),
    year = case_when(
    	category == "OM15" ~ "2015",
    	category == "OM16" ~ "2016"),
    #samples with NR are nitrate only
    compound = ifelse(grepl("NR", name), "NO3", "NO3+NO2"), 
    subrun = ifelse(grepl("NR", name), "subrun 2", "subrun 1"),
...
Note: Run 1 is split into subruns 1 and 2. Subrun 2 is made up of nitrite removed samples and has "NR" labeled in the sample names. The entire run was for a target N concentration of 20 nmoles.
```{r "raw data summary"}
...
Standards look fairly uniform in area and d45. Standards are plotted by subrun. Subrun 1 is samples that contain nitrate and potentially nitrite; subrun 2 contains samples and standards that underwent nitrite removal procedures.
...
### Drift correction
Evaluate drift throughout run duration. Little drift is observed; linear correction applied.
```{r}
...
stds_isotopes <- read_excel(file.path("data", "standards", "standards_run1.xlsx"), sheet = "isotopes")
stds_concs <- read_excel(file.path("data", "standards", "standards_run1.xlsx"), sheet = "concentrations")
tibble(stds_isotopes)
tibble(stds_concs)
```
...
### Check data
A concentration control was not used in calibration, but is calculated to be 165 uM +/- 2 uM. The known value is 161 uM
```{r}
...
write.csv(file = file.path("data", "calibrated", "run1_calib_data_export.csv"))

df_final %>% 
 filter(category %in% c("OM15", "OM16")) %>% 
  saveRDS(file = file.path("data", "calibrated", "df_final_run1"))
# saveRDS
``` 
