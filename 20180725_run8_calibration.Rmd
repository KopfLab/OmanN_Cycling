---
title: 'Run 8: Oman 2018 NO$_x$ $\delta^{15}N$ and $\delta^{18}O$'
subtitle: "Source file: 20180725_run8_calibration.Rmd"
date: "`r Sys.Date()`"
output:
  html_document: 
    css: stylesheet.css
...
#omit any files with problems and export to an .rds file
  raw_data %>%
    ...
    iso_save("data/raw/run8_raw") 
```

### Load raw data from rds file
```{r "load run"}
#load entire run
raw_data <- iso_read_continuous_flow(file.path("data", "raw", "run8_raw.cf.rds"))
```
...
df.raw <- raw_data %>% 
  ...
  left_join(read_excel(file.path("data", "metadata", "180725_category_metadata.xlsx")) %>% select(-expt), by = c("analysis")) %>% 
  ...
  filter(category %in% c("Background", "control", "N2O", "HPW", "Conditioner", "Oman", "USGS-34", "IAEA-NO3"))

#remove volume from N2O analysis 40597
df.raw$volume[df.raw$analysis == 40597] <- NA

df.raw
```
File 40577_KRRCKA_N2O.dxf is the 121st vial- no sample is expected due to the autosampler changing over. No peak detected in 40594_KRRCKA_Background.dxf, the background culture media with culture and no sample added. 
...
With the exception of run_number 38 and 39, standards look uniform in area and d45.
...
```{r "remove problematic data points"}
 df.raw <- df.raw %>% 
 change_category(run_number %in% c(38,39), "excluded") #area too small
```
...
stds_isotopes <- read_excel(file.path("data", "standards", "standards_run8.xlsx"), sheet = "isotopes")
stds_concs <- read_excel(file.path("data", "standards", "standards_run8.xlsx"), sheet = "concentrations")
stds_isotopes
stds_concs
```
...
df_final %>%
  ...
  select(analysis, run_number, volume, area, name, pH, category, dilution, col_NOx, conc, conc_err, d15N, d15N_err, d18O, d18O_err, compound) %>%
write.csv(file = file.path("data", "calibrated", "run8_calib_data_export.csv"))

df_final %>% 
 filter(category == "Oman") %>% 
  saveRDS(file = file.path("data", "calibrated", "df_final_run8"))
# saveRDS
``` 
...
write.csv(file = file.path("data", "calibrated", "run8_stds_calib_data_export.csv"))

df_final %>% 
 filter(category %in% c("USGS-34", "IAEA-NO3")) %>%
  saveRDS(file = file.path("data", "calibrated", "df_stds_run8"))
# saveRDS
```